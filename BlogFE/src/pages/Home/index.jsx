...
import author from '../../states/modules/author';

function Home () {

  const blogs = useSelector(state => state.blog.paginationListBlog)
  const users = useSelector(state => state.employee.paginationListEmployee)
  const authors = useSelector(state => state.author.paginationListAuthor)
  const categorys = useSelector(state => state.category.paginationListCategory)
  return (
    <MainLayout>
